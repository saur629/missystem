...
const SHOP_NAME = typeof window !== 'undefined' ? (process.env.NEXT_PUBLIC_SHOP_NAME || 'PrintFlow MIS') : 'PrintFlow MIS'

// ── A4 Print ──────────────────────────────────────────────────
function printPO(po: any) {
  const items: any[] = (() => { try { return JSON.parse(po.itemsJson || '[]') } catch { return po.items || [] } })()
  const now = new Date()
  const statusColors: Record<string, string> = { ORDERED: '#d97706', IN_TRANSIT: '#2563eb', RECEIVED: '#16a34a', CANCELLED: '#dc2626' }
  ...
  .hdr{display:flex;justify-content:space-between;padding-bottom:12px;border-bottom:3px solid #1a56db;margin-bottom:14px}
  .shop{font-size:20px;font-weight:800;color:#1a56db}.pono{font-size:18px;font-weight:800;color:#1a56db;text-align:right}
  ...
  table{width:100%;border-collapse:collapse;font-size:11px}th{background:#1a56db;color:#fff;padding:6px 8px;text-align:left;font-size:10px;font-weight:700}
  ...
  .trow{display:flex;justify-content:space-between;padding:7px 12px;font-size:12px}.grand{background:#1a56db;color:#fff;font-size:14px;font-weight:800}
  ...
  .sigbox{text-align:center}.sigline{height:30px;border-bottom:1px solid #374151;margin-bottom:4px}.siglabel{font-size:9px;color:#6b7280}
  </style></head><body>
  <div class="hdr"><div><div class="shop">🛒 ${SHOP_NAME}</div><div style="font-size:11px;color:#6b7280">Purchase Order</div></div>
  <div><div class="pono">${po.poNo}</div><div class="meta">Date: <strong>${formatDate(po.date)}</strong></div><div class="meta">Printed: ${now.toLocaleString('en-IN')}</div></div></div>
  <span class="badge">${po.status.replace('_', ' ')}</span>
  <div class="two">
    <div class="ibox"><div class="ilabel">Supplier</div><div class="ival">${po.supplier?.name || '—'}</div></div>
    <div class="ibox"><div class="ilabel">Contact</div><div class="ival" style="color:#1a56db">${po.supplier?.contact || '—'}</div></div>
    <div class="ibox"><div class="ilabel">Total Items</div><div class="ival">${items.length}</div></div>
  ...
  <table><thead><tr><th style="width:32px">Sr.</th><th>Item Name</th><th style="width:60px;text-align:center">Unit</th><th style="width:60px;text-align:center">Qty</th><th style="width:80px;text-align:right">Rate</th><th style="width:90px;text-align:right">Amount</th></tr></thead>
  <tbody>${rows}</tbody></table>
  <div class="totbox"><div class="trow grand"><span>TOTAL AMOUNT</span><span>₹${(po.totalAmount || 0).toLocaleString('en-IN')}</span></div></div>
  ${po.notes ? `<div class="sec">📝 Notes</div><div style="border:1px solid #e5e7eb;border-radius:6px;padding:10px;font-size:11px">${po.notes}</div>` : ''}
  <div class="sigrow"><div class="sigbox"><div class="sigline"></div><div class="siglabel">Prepared By</div></div><div class="sigbox"><div class="sigline"></div><div class="siglabel">Supplier Signature</div></div></div>
  <div class="footer">${SHOP_NAME} • ${po.poNo} • ${now.toLocaleString('en-IN')}</div>
  </body></html>`
...
// ── FormBody OUTSIDE component — prevents focus loss on typing ─
function FormBody({ form, setForm, suppliers, addItem, removeItem, updateItem }: {
  form: any; setForm: any; suppliers: any[]
  addItem: () => void; removeItem: (i: number) => void
  updateItem: (i: number, k: string, v: string) => void
}) {
  const grandTotal = form.items.reduce((s: number, i: any) =>
    s + (parseFloat(i.qty || '0') * parseFloat(i.rate || '0')), 0)
  return (
    <>
      <FormGroup label="Supplier *">
        <Select value={form.supplierId} onChange={e => setForm((p: any) => ({ ...p, supplierId: e.target.value }))} required>
...
export default function PurchasePage() {
  ...
  const updateItem = useCallback((i: number, k: string, v: string) => {
  ...
  function openEdit(po: any) {
    const items = (() => { try { return JSON.parse(po.itemsJson || '[]') } catch { return po.items || [] } })()
    setEditPO(po)
    setForm({
      supplierId: po.supplierId || po.supplier?.id || '',
      notes: po.notes || '',
      items: items.length > 0
        ? items.map((i: any) => ({ itemName: i.itemName || '', unit: i.unit || 'KG', qty: String(i.qty || ''), rate: String(i.rate || '') }))
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setPurchases(p => [po, ...p]); setShowModal(false); resetForm()
  ...
  async function handleEdit(e: React.FormEvent) {
    e.preventDefault(); if (!editPO) return; setSaving(true)
    try {
      const res = await fetch(`/api/purchase/${editPO.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
      if (!res.ok) throw new Error()
      const updated = await res.json()
      setPurchases(p => p.map(po => po.id === updated.id ? updated : po))
      setEditPO(null); resetForm(); toast.success('PO updated!')
    } catch { toast.error('Failed to update PO') }
    setSaving(false)
  }

  async function handleDelete() {
    if (!deletePO) return; setDeleting(true)
    try {
      const res = await fetch(`/api/purchase/${deletePO.id}`, { method: 'DELETE' })
      if (!res.ok) throw new Error()
      setPurchases(p => p.filter(po => po.id !== deletePO.id)); setDeletePO(null); toast.success('PO deleted')
    ...
    <PageShell title="Purchase & Inventory" action={{ label: '+ New PO', onClick: () => setShowModal(true) }}>
            ...
            {loading ? <Loading /> : purchases.length === 0 ? <Empty message="No purchase orders yet" /> : (
              <div style={{ overflowX: 'auto' }}>
                <table>
                  ...
                  <tbody>
                    {purchases.map(po => {
                      const poItems: any[] = (() => { try { return JSON.parse(po.itemsJson || '[]') } catch { return po.items || [] } })()
                      return (
                        <tr key={po.id}>
                          <td style={{ color: '#3b82f6', fontFamily: 'monospace', fontSize: 11, fontWeight: 600 }}>{po.poNo}</td>
                          <td style={{ color: '#8892a4', fontSize: 11, whiteSpace: 'nowrap' }}>{formatDate(po.date)}</td>
                          <td>
                            <div style={{ fontWeight: 600 }}>{po.supplier?.name}</div>
                            <div style={{ fontSize: 10, color: '#8892a4' }}>{po.supplier?.contact}</div>
                          </td>
                          <td style={{ fontSize: 11 }}>
                            {poItems.length > 0 ? (
                              <div>
                                <div style={{ color: '#e2e8f0', fontWeight: 600 }}>{poItems[0]?.itemName}</div>
                                {poItems.length > 1 && <div style={{ fontSize: 10, color: '#8892a4' }}>+{poItems.length - 1} more</div>}
                              </div>
                            ) : <span style={{ color: '#4a5568' }}>—</span>}
                          </td>
                          <td style={{ color: '#10b981', fontWeight: 600 }}>{formatCurrency(po.totalAmount)}</td>
                          <td style={{ color: (po.paidAmount || 0) >= po.totalAmount ? '#10b981' : '#f59e0b', fontSize: 11 }}>
                            {formatCurrency(po.paidAmount || 0)}
                          </td>
                          <td><Badge color={PO_COLOR[po.status]}>{po.status.replace('_', ' ')}</Badge></td>
                          <td>
                            <div style={{ display: 'flex', gap: 4 }}>
                              <button onClick={() => setViewPO({ ...po, parsedItems: (() => { try { return JSON.parse(po.itemsJson || '[]') } catch { return po.items || [] } })() })}
                                style={{ padding: '4px 8px', background: 'rgba(59,130,246,.1)', border: '1px solid rgba(59,130,246,.25)', borderRadius: 6, color: '#3b82f6', fontSize: 12, cursor: 'pointer' }}>👁</button>
                              <button onClick={() => openEdit(po)}
                                style={{ padding: '4px 8px', background: 'rgba(16,185,129,.1)', border: '1px solid rgba(16,185,129,.25)', borderRadius: 6, color: '#10b981', fontSize: 12, cursor: 'pointer' }}>✏️</button>
                              <button onClick={() => setDeletePO(po)}
                                style={{ padding: '4px 8px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.25)', borderRadius: 6, color: '#ef4444', fontSize: 12, cursor: 'pointer' }}>🗑️</button>
                            </div>
                          </td>
                        </tr>
                      )
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); resetForm() }} title="🛒 New Purchase Order"
        ...
        <form onSubmit={handleCreate}>
          <FormBody form={form} setForm={setForm} suppliers={suppliers} addItem={addItem} removeItem={removeItem} updateItem={updateItem} />
        </form>
      </Modal>
      ...
      {/* ── VIEW MODAL ── */}
      <Modal open={!!viewPO} onClose={() => setViewPO(null)} title={`${viewPO?.poNo} — ${viewPO?.supplier?.name}`} width={580}
        footer={
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', gap: 8 }}>
            <button onClick={() => printPO(viewPO)} style={{ padding: '7px 16px', background: 'rgba(59,130,246,.15)', border: '1px solid rgba(59,130,246,.4)', borderRadius: 8, color: '#3b82f6', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              🖨️ Print A4
            </button>
        ...
        {viewPO && (
          <div>
            ...
            <div style={{ fontSize: 11, fontWeight: 700, color: '#e2e8f0', marginBottom: 8 }}>📦 Items ({viewPO.parsedItems?.length || 0})</div>
            <div style={{ overflowX: 'auto', marginBottom: 12 }}>
              <table>
                <thead><tr><th>#</th><th>Item Name</th><th style={{ textAlign: 'center' }}>Unit</th><th style={{ textAlign: 'center' }}>Qty</th><th style={{ textAlign: 'right' }}>Rate</th><th style={{ textAlign: 'right' }}>Amount</th></tr></thead>
                <tbody>
                  {viewPO.parsedItems?.map((item: any, i: number) => (
                    <tr key={i}>
                      ...
                      <td style={{ textAlign: 'right', color: '#10b981', fontWeight: 700 }}>₹{(parseFloat(item.qty || 0) * parseFloat(item.rate || 0)).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {viewPO.notes && (
              <div style={{ marginTop: 12, background: '#1e2535', border: '1px solid #2a3348', borderRadius: 8, padding: 10, fontSize: 12, color: '#8892a4' }}>📝 {viewPO.notes}</div>
            )}
          </div>
        )}
      </Modal>
      ...
      <Modal open={!!deletePO} onClose={() => setDeletePO(null)} title="🗑️ Delete Purchase Order"
        footer={<><Button onClick={() => setDeletePO(null)}>Cancel</Button>
          <button onClick={handleDelete} disabled={deleting} style={{ padding: '8px 18px', background: 'rgba(239,68,68,.15)', border: '1px solid rgba(239,68,68,.3)', borderRadius: 8, color: '#ef4444', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
        ...
        {deletePO && (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <div style={{ marginTop: 12, fontSize: 12, color: '#ef4444', background: 'rgba(239,68,68,.08)', borderRadius: 8, padding: '8px 14px' }}>This cannot be undone.</div>
          </div>
        )}
      </Modal>
    </PageShell>
