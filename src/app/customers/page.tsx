...
import { Card, CardHeader, CardTitle, Badge, Button, Modal, FormGroup, Input, StatCard, Loading, Empty, Grid } from '@/components/ui'
...
export default function CustomersPage() {
  ...
  const [form, setForm] = useState({ name:'', mobile:'', email:'', address:'', city:'', gstNo:'', creditLimit:'' })
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setCustomers(p => [...p, c])
      setShowModal(false)
      setForm({ name:'', mobile:'', email:'', address:'', city:'', gstNo:'', creditLimit:'' })
    ...
    <PageShell title="Customers" action={{ label:'+ Add Customer', onClick:()=>setShowModal(true) }}>
      <div className="animate-in">
        ...
        <div style={{ display:'flex', gap:8, marginBottom:14 }}>
          <Input placeholder="🔍 Search by name or mobile..." value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==='Enter'&&fetchCustomers()} style={{ flex:1 }} />
        ...
        <Card>
          ...
          {loading ? <Loading /> : filtered.length===0 ? <Empty message="No customers found" /> : (
            <div style={{ overflowX:'auto' }}>
              <table>
                <thead><tr><th>Code</th><th>Name</th><th>Mobile</th><th>Email</th><th>City</th><th>GST No.</th><th>Credit Limit</th><th>Status</th></tr></thead>
                <tbody>
                  {filtered.map(c => (
                    <tr key={c.id}>
                      ...
                      <td style={{ color:'#8892a4', fontSize:11 }}>{c.email||'—'}</td>
      ...
      <Modal open={showModal} onClose={()=>setShowModal(false)} title="Add New Customer"
        footer={<><Button onClick={()=>setShowModal(false)}>Cancel</Button><Button variant="primary" onClick={handleCreate} disabled={saving}>{saving?'Saving...':'💾 Save Customer'}</Button></>}>
        <form onSubmit={handleCreate}>
          <Grid cols={2} gap={12}>
            <FormGroup label="Name *"><Input value={form.name} onChange={e=>f('name',e.target.value)} required /></FormGroup>
            <FormGroup label="Mobile *"><Input value={form.mobile} onChange={e=>f('mobile',e.target.value)} required /></FormGroup>
            <FormGroup label="Email"><Input type="email" value={form.email} onChange={e=>f('email',e.target.value)} /></FormGroup>
            <FormGroup label="City"><Input value={form.city} onChange={e=>f('city',e.target.value)} /></FormGroup>
            <FormGroup label="GST No."><Input value={form.gstNo} onChange={e=>f('gstNo',e.target.value)} placeholder="09XXXXX1234Z1ZX" /></FormGroup>
            <FormGroup label="Credit Limit (₹)"><Input type="number" value={form.creditLimit} onChange={e=>f('creditLimit',e.target.value)} /></FormGroup>
          </Grid>
          <FormGroup label="Address"><Input value={form.address} onChange={e=>f('address',e.target.value)} /></FormGroup>
        </form>
      </Modal>
    </PageShell>
  )
