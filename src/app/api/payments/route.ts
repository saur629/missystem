...
export async function GET(req: NextRequest) {
  ...
  // For customer-specific queries (ledger), return all without pagination
  if (customerId) {
    ...
    if (search) {
      ...
      return NextResponse.json(payments.filter((p: any) =>
        String(p.receiptNo||'').toLowerCase().includes(s) ||
  ...
  // Paginated for main payments list
    ...
    try {
      ...
      const creditPayment = await prisma.payment.create({
        data: {
          ...
          amount:   applyAmt,
          mode:     'CREDIT',
          notes:    `Credit applied to ${targetOrder.orderNo}`,
          date:     new Date(),
          type:     'CREDIT_APPLIED',
        },
      ...
      return NextResponse.json({ error: err.message || 'Failed to apply credit' }, { status: 500 })
    }
  }
