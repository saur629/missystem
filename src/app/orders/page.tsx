...
<head>
 ...
 <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;padding-bottom:10px;border-bottom:3px solid #1a56db}
    .shop{font-size:18px;font-weight:800;color:#1a56db}
    .sub{font-size:11px;color:#6b7280;margin-top:2px}
    .right-header{text-align:right}
    .order-no{font-size:17px;font-weight:800;color:#1a56db;font-family:monospace}
    .meta{font-size:10px;color:#6b7280;margin-top:2px}

    .status-bar{display:flex;align-items:center;gap:8px;padding:6px 12px;border-radius:6px;margin-bottom:10px;border:1px solid #e5e7eb;background:#f9fafb}
    .status-pill{padding:3px 12px;border-radius:20px;font-size:10px;font-weight:800;color:#fff}
    .priority-pill{padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700}
    ...
    .info-box{border:1px solid #e5e7eb;border-radius:6px;padding:7px 10px}
    .info-label{font-size:8px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px}
    ...
    .section{font-size:9px;font-weight:700;color:#1a56db;background:#eff6ff;border-left:4px solid #1a56db;padding:4px 8px;margin:10px 0 6px;border-radius:0 4px 4px 0;text-transform:uppercase;letter-spacing:.5px}

    table{width:100%;border-collapse:collapse;font-size:10px;margin-bottom:10px;table-layout:fixed}
    th{background:#1a56db;color:#fff;padding:6px 5px;text-align:left;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.3px;word-break:break-word}
    td{padding:6px 5px;border-bottom:1px solid #f3f4f6;vertical-align:middle;word-break:break-word;overflow-wrap:break-word}
    tr:nth-child(even) td{background:#f9fafb}

    .bottom-section{display:grid;grid-template-columns:1fr 200px;gap:12px;margin-bottom:10px;align-items:start}

    .qr-section{display:flex;align-items:flex-start;gap:12px;padding:10px;border:1px solid #e5e7eb;border-radius:6px;background:#f9fafb}
    .qr-img{width:80px;height:80px;flex-shrink:0;border:2px solid #e5e7eb;border-radius:4px}
    .qr-info{flex:1}
    .qr-title{font-size:9px;font-weight:700;color:#6b7280;text-transform:uppercase;margin-bottom:4px}
    .qr-line{font-size:9px;color:#374151;margin-bottom:2px}

    .totals-table{border:1px solid #e5e7eb;border-radius:6px;overflow:hidden}
    .t-row{display:flex;justify-content:space-between;padding:6px 10px;border-bottom:1px solid #f3f4f6;font-size:11px}
    .t-row:last-child{border-bottom:none}
    .t-row.grand{background:#1a56db;color:#fff;font-size:13px;font-weight:800}
    .t-row.balance-row{font-weight:700;font-size:12px}
    .t-label{color:#6b7280}
    .t-val{font-weight:600}

    .notes-box{border:1px solid #e5e7eb;border-radius:6px;padding:8px 10px;min-height:30px;font-size:10px;color:#374151;margin-bottom:10px;background:#fafafa}

    .workflow{display:flex;margin-bottom:10px;gap:0}
    .wf-step{flex:1;text-align:center;font-size:7px;font-weight:700;border-top:3px solid #e5e7eb;padding-top:4px;color:#9ca3af}
...
<body>
  ...
  <div class="workflow">
    ${[
      ...
      .map((s) => {
        ...
        const ci = all.indexOf(order.status),
          ti = all.indexOf(s);
  ...
  <div class="bottom-section">
    ...
    <!-- QR CODE -->
    <div class="qr-section" style="height:100%">
...
// ─── FlexItemRow ──────────────────────────────────────────────────────────────
const FlexItemRow = memo(function FlexItemRow({
  item,
  idx,
  total,
  onChange,
    ...
    >
          ...
          >
            {(["ft", "inches"] as const).map((u) => (
              <button
                key={u}
                type="button"
                onClick={() => onChange(item.id, "unit", u)}
                style={{
                  padding: "3px 10px",
                  fontSize: 10,
                  cursor: "pointer",
                  fontWeight: item.unit === u ? 700 : 400,
                  background: item.unit === u ? "#3b82f6" : "transparent",
                  color: item.unit === u ? "#fff" : "#8892a4",
                  border: "none",
                }}
              >
                {u}
      ...
      >
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => onChange(item.id, "ratePerSqFt", e.target.value)}
            placeholder="0.00"
          />
        </div>
        <div>
      ...
      >
        <div>
          ...
          <Select
            value={item.flexMedia}
            onChange={(e) => onChange(item.id, "flexMedia", e.target.value)}
          >
            {FLEX_MEDIA.map((m) => (
        ...
        <div>
          ...
          <Input
            ...
            placeholder="e.g. Shop Banner, Left Side"
          />
        </div>
        <div>
          <div
      ...
      {parseFloat(item.designCharge || "0") > 0 && (
        ...
        >
          ...
          >
            🎨 Design: ₹{parseFloat(item.designCharge).toLocaleString("en-IN")}{" "}
            included in amount
          </span>
        </div>
      )}
...
// ─── PrintItemRow ─────────────────────────────────────────────────────────────
const PrintItemRow = memo(function PrintItemRow({
  item,
  idx,
  total,
  onChange,
    ...
    >
      ...
      >
        <div>
          ...
          <Input
            value={item.jobName}
            onChange={(e) => onChange(item.id, "jobName", e.target.value)}
      ...
      >
        ...
        <div>
          ...
          <Select
            value={item.colors}
            onChange={(e) => onChange(item.id, "colors", e.target.value)}
          >
            {["1-color", "2-color", "4-color (CMYK)", "Spot Color"].map((c) => (
        ...
        <div>
          ...
          <Select
            ...
            onChange={(e) => onChange(item.id, "printSide", e.target.value)}
          >
        ...
        <div>
          ...
          <Select
            ...
            onChange={(e) => onChange(item.id, "lamination", e.target.value)}
          >
            <option value="">None</option>
            {["Matt Lam", "Glossy Lam", "Soft Touch", "UV Coating"].map((l) => (
    ...
    <>
      ...
      <Grid cols={3} gap={12}>
        ...
        <FormGroup label="GST %">
          <Select
            value={form.gstPct}
            onChange={(e) => onFormChange("gstPct", e.target.value)}
          >
        ...
        <FormGroup label="Advance Paid ₹">
          <Input
            ...
            onChange={(e) => onFormChange("advancePaid", e.target.value)}
          />
        </FormGroup>
      </Grid>
      <div
      ...
      >
        <div
          style={{
            display: "grid",
            gridTemplateColumns:
              profit !== null ? "1fr 1fr 1fr 1fr 1fr" : "1fr 1fr 1fr 1fr",
            gap: 12,
          }}
        >
          <div>
            ...
            <div style={{ fontWeight: 600 }}>{formatCurrency(afterDisc)}</div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: "#8892a4", marginBottom: 3 }}>
              GST {form.gstPct}%
            </div>
            <div style={{ fontWeight: 600 }}>{formatCurrency(gstAmt)}</div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: "#8892a4", marginBottom: 3 }}>
              Total
...
export default function OrdersPage() {
    ...
    setItems((prev) =>
      prev.map((item) => {
        ...
        if (key === "sellingPrice" || key === "qty" || key === "designCharge") {
          const sp =
            parseFloat(key === "sellingPrice" ? val : updated.sellingPrice) ||
            0;
          const q = parseInt(key === "qty" ? val : updated.qty) || 0;
          const dc =
            parseFloat(
              key === "designCharge" ? val : updated.designCharge || "0",
            ) || 0;
        ...
        p.map((o) =>
          o.id === updated.id
            ? {
                ...
                orderItems: (() => {
                  try {
                    return JSON.parse(updated.orderItemsJson || "[]");
                  } catch {
                    return [];
                  }
                })(),
  ...
  async function saveItemStatuses(orderId: string, updatedItems: any[]) {
    ...
    if (res.ok) {
      setOrders((p) =>
        p.map((o) =>
          o.id === orderId
            ? { ...o, orderItemsJson: JSON.stringify(updatedItems) }
            : o,
        ),
      );
  ...
  return (
    <PageShell
      ...
      action={
        canBook
          ? { label: "+ New Order", onClick: () => setShowModal(true) }
          : undefined
      }
    >
      <div className="animate-in">
        ...
        >
          <Input
            placeholder="🔍 Search name, mobile, order no..."
            value={search}
              ...
              <table>
                ...
                <tbody>
                  {orders.map((o) => {
                    ...
                    const allPrint =
                      pItems.length > 0 &&
                      pItems.every((i: any) => i.printStatus === "DONE");
                    return (
                      <tr key={o.id}>
                        <td
                          style={{
                          ...
                          >
                            <span
                              style={{
                                ...
                                padding: "1px 5px",
                                borderRadius: 4,
                                background: allDesign
                                  ? "rgba(16,185,129,.15)"
                                  : "rgba(245,158,11,.1)",
                                color: allDesign ? "#10b981" : "#f59e0b",
                              }}
                            >
                              🎨 {allDesign ? "Done" : "Design"}
                            </span>
                            <span
                              style={{
                                fontSize: 9,
                                ...
                                background: allPrint
                                  ? "rgba(16,185,129,.15)"
                                  : "rgba(139,92,246,.1)",
                                color: allPrint ? "#10b981" : "#8b5cf6",
                              }}
                            >
                              🖨️ {allPrint ? "Done" : "Print"}
                            </span>
                          </div>
                        </td>
        ...
        footer={
          <>
            ...
            <Button variant="primary" onClick={handleCreate} disabled={saving}>
              {saving
                ? "Saving..."
                : `💾 Save Order (${items.length} item${items.length > 1 ? "s" : ""})`}
            </Button>
          </>
        }
      >
        <form onSubmit={handleCreate}>
          <OrderFormBody {...formProps} />
        </form>
      </Modal>
      ...
      <Modal
        ...
        title={`✏️ Edit — ${editOrder?.orderNo}`}
        width={700}
        footer={
          <>
            <Button
      ...
      <Modal
        ...
        title={`${viewOrder?.orderNo} — ${viewOrder?.customer?.name}`}
        width={700}
        footer={
          <div
            style={{
          ...
          >
            <Button
              onClick={() => printOrderSummary({ ...viewOrder }, shopName)}
              style={{
      ...
      >
        {viewOrder &&
          (() => {
            ...
            const setVItems = (updated: any[]) =>
              setViewOrder((v: any) => ({ ...v, orderItems: updated }));
            return (
              <div>
                <Grid cols={3} gap={14} style={{ marginBottom: 16 }}>
                  ...
                  <InfoBox
                    label="Total"
                    value={formatCurrency(viewOrder.totalAmount)}
                    color="#10b981"
                  />
                  <InfoBox
                    label="Advance"
                    value={formatCurrency(viewOrder.advancePaid)}
                    color="#3b82f6"
                  />
                  <InfoBox
                    label="Balance"
                    value={formatCurrency(viewOrder.balanceDue)}
                    color={viewOrder.balanceDue > 0 ? "#ef4444" : "#10b981"}
                  />
                  <InfoBox
                    label="Due Date"
                    value={
                      viewOrder.dueDate ? formatDate(viewOrder.dueDate) : "—"
                    }
                  />
                  <InfoBox
                    label="GST"
                    value={`${viewOrder.gstPct}% = ${formatCurrency(viewOrder.gstAmount)}`}
                  />
                  <InfoBox
                    label="Discount"
                  ...
                  <div style={{ marginBottom: 16 }}>
                                  ...
                                  {TASK_STATUSES.map((s) => (
                                    <button
                                      ...
                                      style={{
                                        ...
                                        cursor: "pointer",
                                        fontWeight:
                                          item[field] === s ? 700 : 400,
                                        border: `1px solid ${item[field] === s ? STATUS_COLOR[s] : "#2a3348"}`,
                                        background:
                                          item[field] === s
                                            ? `${STATUS_COLOR[s]}22`
                                            : "#252d40",
                    ...
                    >
                      <Button
                        variant="primary"
                        size="sm"
                        onClick={() => saveItemStatuses(viewOrder.id, vItems)}
                      >
                        💾 Save Task Statuses
                      </Button>
                    </div>
                  ...
                  {Object.entries(ORDER_STATUS).map(([key, val]) => (
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 12,
                        cursor: "pointer",
                        border: `1px solid ${viewOrder.status === key ? "#3b82f6" : "#2a3348"}`,
                        background:
                          viewOrder.status === key
                            ? "rgba(59,130,246,.15)"
                            : "#1e2535",
                        color: viewOrder.status === key ? "#3b82f6" : "#8892a4",
                        fontWeight: viewOrder.status === key ? 700 : 400,
                      }}
                    >
      ...
      <Modal
        ...
        footer={
          <>
            <Button onClick={() => setShowCustModal(false)}>Cancel</Button>
        ...
        <form onSubmit={handleAddCustomer}>
          <FormGroup label="Name *">
            <Input
              value={custForm.name}
              onChange={(e) =>
                setCustForm((p) => ({ ...p, name: e.target.value }))
              }
              required
            />
          </FormGroup>
          <FormGroup label="Mobile *">
            <Input
              ...
              onChange={(e) =>
                setCustForm((p) => ({ ...p, mobile: e.target.value }))
              }
              required
            />
          </FormGroup>
          <Grid cols={2} gap={12}>
            <FormGroup label="Email">
              <Input
                ...
                onChange={(e) =>
                  setCustForm((p) => ({ ...p, email: e.target.value }))
                }
              />
            </FormGroup>
            <FormGroup label="City">
              <Input
                value={custForm.city}
                onChange={(e) =>
                  setCustForm((p) => ({ ...p, city: e.target.value }))
                }
              />
            </FormGroup>
          </Grid>
          <FormGroup label="GST No.">
            <Input
              ...
              onChange={(e) =>
                setCustForm((p) => ({ ...p, gstNo: e.target.value }))
              }
