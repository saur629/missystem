...
<body>
  ...
  <!-- WORKFLOW STATUS BAR -->
  <div class="workflow">
    ${['PENDING','DESIGNING','DESIGN_DONE','PRINTING','PRINT_DONE','QUALITY_CHECK','READY','DELIVERED'].map(s => {
      const statuses = ['PENDING','DESIGNING','DESIGN_DONE','PRINTING','PRINT_DONE','QUALITY_CHECK','READY','DELIVERED']
      const currentIdx = statuses.indexOf(order.status)
      const thisIdx = statuses.indexOf(s)
      const cls = s === order.status ? 'active' : thisIdx < currentIdx ? 'done' : ''
      const labels: Record<string,string> = { PENDING:'Booked', DESIGNING:'Design', DESIGN_DONE:'Design✓', PRINTING:'Printing', PRINT_DONE:'Print✓', QUALITY_CHECK:'QC', READY:'Ready', DELIVERED:'Delivered' }
  ...
  <div class="info-grid">
    ...
    <div class="info-box">
      ...
      <div class="info-value">${order.customer?.mobile || '—'}</div>
    </div>
    <div class="info-box">
    ...
    <thead>
      <tr>
        ...
        ${order.orderType === 'FLEX'
          ? `<th>Description / Label</th><th>Media</th><th>Width (ft)</th><th>Height (ft)</th><th>Sq.Ft</th><th>Qty</th><th>Rate/sqft</th><th style="text-align:right">Amount</th>`
        ...
        : `<tr>
            ...
            ${order.orderType === 'FLEX'
              ? `<td><strong>${order.description || '—'}</strong></td>
                 ...
                 <td style="text-align:center;font-weight:600">${order.width || '—'}</td>
                 <td style="text-align:center;font-weight:600">${order.height || '—'}</td>
  ...
  <div class="checklist">
    ...
    <div class="check-item"><div class="checkbox"></div> Media loaded — ${order.flexMedia || 'check media type'}</div>
    <div class="check-item"><div class="checkbox"></div> Size verified — ${items.length > 0 ? `${items.length} banners` : `${order.width}×${order.height}ft`}</div>
      ...
      <div style={{ marginBottom: 8 }}>
        {items.length > 0 ? items.map((item: any, i: number) => (
          <div key={i} style={{ fontSize: 11, padding: '3px 7px', background: 'rgba(59,130,246,.08)', borderRadius: 5, marginBottom: 3, color: '#93c5fd' }}>
            {order.orderType === 'FLEX'
              ? `${item.description || `Item ${i+1}`}: ${item.width || 0}×${item.height || 0}ft = ${item.sqFt ? parseFloat(item.sqFt).toFixed(1) : 0} sqft × qty ${item.qty || 1}`
